// setup to save preferences in flash
#include <Preferences.h>
Preferences preferences;  // At global scope
...
using namespace firebase;  //NOTE, HAD TO DOWNGRADE TO FIX COMPILE ERROR
...
const float SHAKE_THRESHOLD = 30.0;              // Threshold for shake detection
const int MAX_MESSAGE_LENGTH = 1000;             // Maximum message length

// WiFi and Email Configuration
const char* WIFI_SSID = "StormSurge";
const char* WIFI_PASSWORD = "PhoebeAndFreya25!";

const char* DATABASE_URL = "https://hugs-for-alan-default-rtdb.firebaseio.com/";

// Constants for timezone conversion
const int PST_OFFSET = -8;  // West Coast Time is UTC-8 (PST)
const int SECONDS_PER_HOUR = 3600;

// define global objects
Adafruit_MAX17048 lipo;
Adafruit_ST7789 tft = Adafruit_ST7789(TFT_CS, TFT_DC, TFT_RST);  // screen
...
void formatTimestamp(const String& timestampStr, char* formattedTime) {
  ...
  // Apply EST offset
  timeStamp += (PST_OFFSET * SECONDS_PER_HOUR);
...
void drawTimestamp(const String& timestamp, int x, int y) {
  ...
  int textWidth = strlen(formattedTime) * 12;
...
void displayMailNotice() {
  ...
  // Center "You've Got"
  const char* line1 = "You've Got Mail!";
  int16_t x1, y1;
  uint16_t w1, h1;
  tft.getTextBounds(line1, 0, 0, &x1, &y1, &w1, &h1);
  tft.setCursor((tft.width() - w1) / 2, 30);
  tft.println(line1);

  // // Center "Shake to read"
  // const char* line2 = "(shake me!)";
  // int16_t x2, y2;
  // uint16_t w2, h2;
...
void displayMessage(const String& message) {
  ...
  for (int i = 0; i < EMOJI_MAP_SIZE; i += 2) {
    ...
    while (pos >= 0) {
      processedMessage = processedMessage.substring(0, pos) + String((char)(1 + (i / 2))) + processedMessage.substring(pos + emojiText.length());
  ...
  int startY = (SCREEN_HEIGHT - totalTextHeight) / 2 - CHAR_HEIGHT; //updated to off-center
    ...
    for (int i = 0; i < lineCount; i++) {
      ...
      for (int j = 0; j < line.length(); j++) {
        ...
        if (c >= 1 && c <= EMOJI_MAP_SIZE / 2) {
          ...
          int textY = currentY + ((LINE_HEIGHT - CHAR_HEIGHT) / 2);  // 8 is text height
  ...
  if (!isScrolling) {
    ...
    drawTimestamp(currentTimestamp,
                  ...
                  SCREEN_HEIGHT - 20);  // 10 pixels from bottom
  }
  ...
  if (!scrollState.isInitialized) {
    ...
    while (tempMessage.length() > 0) {
      ...
      if (spaceIndex == -1 && newlineIndex == -1) {
        ...
        int nextBreak = (spaceIndex == -1) ? newlineIndex : (newlineIndex == -1) ? spaceIndex
...
void setup() {
  ...
  Serial.print(F("Hello! Hugs for Alan test"));
